...
interface IgnoreList {
    tokens: IgnoreListToken[]
}

export class ProviderIgnore extends Provider {
    constructor(
    ...
    async getTokens(): Promise<TokenSet> {
        ...
        for (let i = 0; i < tokens.data.tokens.length; i++) {
            const token: IgnoreListToken = tokens.data.tokens[i]
        ...
        console.log(`[IL] Loaded tokens from ${this.url} - ${this.chainId}`)
        return tokenMap
    }
}
